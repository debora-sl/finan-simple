...
| **III. Server Actions Seguras** | ✅ Todas as mutações **de domínio** (despesas, categorias) usam next-safe-action em `actions/`, com `protectedActionClient` (de `lib/action-client.ts`) e `.inputSchema` (nunca `.schema`). Cada action valida autenticação/autorização (posse do recurso pelo usuário). Cliente chama via hook `useAction`. **Exceção esperada**: as mutações de autenticação (cadastro e login) são inerentemente pré-autenticação e pertencem ao Better Auth — passam pelo endpoint `app/api/auth/[...all]/route.ts` e são chamadas via `authClient.signUp/signIn`, não via `protectedActionClient`/`useAction`. Isso não viola o Princípio III (que rege as actions de domínio protegidas), apenas delimita seu escopo. |
